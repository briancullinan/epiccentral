...
namespace ActiveUp.Net.Mail
{
    ...
    public class Imap4Client : System.Net.Sockets.TcpClient
    {
        ...
        public string Command(string command, string stamp, CommandOptions options = null)
        {
            ...
            while (true)
            {
                ...
                else if (temp != null)
                {
                    ...
                    if(temp.ToLower().StartsWith("* BYE"))
                    {
                        throw new Exception("Server disconnected unexpectedly with: " + temp);
                    }
                }
            }
        ...
        public Mailbox SelectMailbox(string mailboxName)
        {
            ...
            try
            {
                var line = ActiveUp.Net.Mail.Imap4Client.FindLine(lines, "[UNSEEN ");
                if(line.Split(' ').Length > 3)
                    unseen = System.Convert.ToInt32(line.Split(' ')[3].TrimEnd(']'));
            }
            catch (Exception) { }
