...
namespace AE.Net.Mail
{
    public class ImapClient : TextClient, IMailClient
    {
        ...
        public ImapClient(string host, string username, string password, AuthMethods method = AuthMethods.Login,
                          int port = 143, bool secure = false, bool skipSslValidation = false)
        {
        ...
        {
            get { return _MessageDeleted != null || _NewMessage != null; }
        }
        ...
        public MailMessage GetMessage(int index, bool headersonly, bool setseen, string[] fields = null)
        {
            return GetMessages(index, index, headersonly, setseen, fields).FirstOrDefault();
        }

        public MailMessage GetMessage(string uid, bool headersonly, bool setseen, string[] fields = null)
        {
            return GetMessages(uid, uid, headersonly, setseen, fields).FirstOrDefault();
        }

        public MailMessage[] GetMessages(string startUID, string endUID, bool headersonly = true, bool setseen = false, string[] fields = null)
        {
            return GetMessages(startUID, endUID, true, headersonly, setseen, fields);
        }

        public MailMessage[] GetMessages(int startIndex, int endIndex, bool headersonly = true, bool setseen = false, string[] fields = null)
        {
            return GetMessages((startIndex + 1).ToString(), (endIndex + 1).ToString(), false, headersonly, setseen, fields);
        }
        ...
        public void Store(string messageset, bool replace, string flags)
        {
            ...
            string command = string.Concat(GetTag(), prefix, "STORE ", messageset, " ", replace ? "" : "+",
                                           "FLAGS.SILENT (" + flags + ")");
            string response = SendCommandGetResponse(command);
            while (response.StartsWith("*"))
            {
